-- -- 文件：~/.config/nvim/lua/myfindreplace/init.lua
--
-- local M = {}
--
-- -- 高亮组名
...
-- -- local script_path = "lua.configs.utility.findreplace"
--
-- -- 创建高亮组
...
--
-- -- 切换输入框
-- function M.switch_input()
--   local cursor = vim.api.nvim_win_get_cursor(M.input_win)
--   if cursor[1] == 1 then
--     -- 切换到第二个输入框
--     vim.api.nvim_win_set_cursor(M.input_win, { 3, 9 })
--   else
--     -- 切换回第一个输入框
--     vim.api.nvim_win_set_cursor(M.input_win, { 1, 7 })
--   end
--   vim.cmd [[startinsert]]
-- end
--
...
--
--   -- 执行查找和替换
--   M.global_replace(args[1], args[2])
-- end
--
-- -- 查找并替换函数
...
--
--       -- 高亮匹配的部分
--       vim.api.nvim_buf_add_highlight(buffer, -1, highlight_group, i - 1, s - 1, e)
--
--       -- 进行替换
